...
function isPluginMarker(part) {
    return part.type === "text" && part.metadata?.[MARKER_METADATA_KEY] === true;
}
function isCompactionControl(message) {
...
function markerPart(message, text) {
    return {
 ...
 */
function addConversationDateMarkers(messages, options) {
    ...
    for (const message of messages) {
        ...
        if (date === today) {
            ...
            if (segmentHasHistoricalGroup)
                marker = `<conversation_date current="true"/>`;
        }
        else {
        ...
        if (marker)
            message.parts.unshift(markerPart(message, marker));
        previousDate = date;
    }
}
...
function updateSystemPrompt(system, options) {
    const now = options.now ?? Date.now();
    ...
    for (let index = 0; index < system.length; index++) {
        ...
        if (entry === undefined || !existingDate.test(entry))
            continue;
        system[index] = entry.replace(existingDate, (_match, indentation) => `${indentation}${dateLine}`);
    }
    if (!system.includes(SYSTEM_INSTRUCTION))
        system.push(SYSTEM_INSTRUCTION);
}
/** Add explicit time context and retention rules to OpenCode's summary prompt. */
function addCompactionTemporalContext(context, options) {
    const now = options.now ?? Date.now();
    const current = calendarDate(now, options.timeZone);
...
const server = (async (_input, rawOptions) => {
    ...
    return {
        "experimental.chat.messages.transform": async (_input, output) => {
            addConversationDateMarkers(output.messages, { timeZone });
        },
